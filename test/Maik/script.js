...
// ============================================================
// SCB DASHBOARD – Linjediagram 2020–2024
// ============================================================
...
async function init() {
  try {
    ...
    const gamla = parseGamla(jsonGamla);
    const nya   = parseNya(jsonNya);
    parsedData  = merge(gamla, nya);
    renderAll(parsedData);
  } catch (e) {
    const el = document.getElementById('chart-main');
    if (el) el.innerHTML = `<div class="chart-error">⚠ Kunde inte hämta data: ${e.message}</div>`;
  }
}
