...
const URL_NYA =
  "https://statistikdatabasen.scb.se/api/v2/tables/TAB6768/data?lang=sv&valueCodes[Forpackning]=10,25,35,40,45,55,65,70,99&valueCodes[ContentsCode]=000008G6,000008G5,00000881&valueCodes[Tid]=2024";
...
async function init() {
  try {
    ...
    if (el) {
      el.innerHTML = `<div class="chart-error">⚠ Kunde inte hämta data: ${e.message}</div>`;
    }
  }
}
...
async function initKorrelation() {
  ...
  try {
    ...
    if (punkter.length < 2) {
      korrelCanvas.insertAdjacentHTML(
        "afterend",
